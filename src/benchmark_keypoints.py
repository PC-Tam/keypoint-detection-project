...
def run_benchmark(config_path: str = "config.yaml", n_sample: int = 200) -> pd.DataFrame:
    ...
    sampled = df_meta.groupby("class_name").sample(
        n=n_per_class, replace=True, random_state=random_state
    ).drop_duplicates(subset=["image_path"]).reset_index(drop=True)
    
    # If we need exactly n_sample, we can sample again
    if len(sampled) > n_sample:
        sampled = sampled.sample(n=n_sample, random_state=random_state).reset_index(drop=True)
