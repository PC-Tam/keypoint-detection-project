...
## 6.1. Pipeline Tiền Xử Lý | Preprocessing Pipeline

Hệ thống tích hợp pipeline tiền xử lý ảnh để cải thiện kết quả keypoint detection và ORB matching, đặc biệt với ảnh thực tế (thiếu sáng, mờ, nhiễu, khác kích thước).

| Bước | Mặc định | Mô tả |
|:---|:---:|:---|
| Resize giữ tỉ lệ | ✅ | Giới hạn kích thước ảnh lớn (max 1024px) |
| Grayscale | ✅ | Chuyển ảnh 1 kênh cho detector |
| CLAHE | ✅ | Cải thiện tương phản cục bộ |
| Gaussian Blur | ❌ | Giảm nhiễu nhẹ (tùy chọn) |
| Median Blur | ❌ | Giảm nhiễu muối tiêu (tùy chọn) |
| Contrast Stretching | ❌ | Kéo giãn histogram (tùy chọn) |
| Sharpen | ❌ | Làm rõ cạnh (tùy chọn) |

**Không dùng mặc định**: Thresholding (phá texture/descriptor), Global Histogram Equalization (khuếch đại nhiễu).

> Chi tiết kỹ thuật: xem [`docs/PREPROCESSING_PIPELINE.md`](docs/PREPROCESSING_PIPELINE.md)
